% Use like pop_loadxdf (programatically) 
function EEG = pop_loadxdf_nosync(fullFileName, varargin)

EEG = eeg_load_xdf_nosync(fullFileName, varargin{:});
EEG = eeg_checkset(EEG);
end

function raw = eeg_load_xdf_nosync(filename, varargin)
...
% GUSTAVO: I have set 'HandleClockSynchronization' to 0 because it was unsynchronizing the data + markers sent by NeuXus and stored by Lab Recorded 
% An option to set it to 0 in the official pop_loadxdf would be great.
...
for s=1:length(streams)
    ...
    % if (strcmp(streams{s}.info.type,'marker') || strcmp(streams{s}.info.type,'Events')) && ~ismember(streams{s}.info.name,args.exclude_markerstreams)
        try
